	...
	<div class="project-preview-donation m-b-lg">
	  <div class="wrapper">
		<div class="project-preview-donation-title">
			Собрано <?=number_format($arResult['PROPERTIES']['COLLECTED']['VALUE'], 0, '.', ' ')?> руб из <?=number_format($arResult['PROPERTIES']['GOAL']['VALUE'], 0, '.', ' ')?> руб<?if($arResult['PROPERTIES']['ANNUAL']['VALUE']=="Y"):?>/год<?endif;?>
		</div>
		...
		<?if($arResult['PROPERTIES']['ANNUAL']['VALUE']=="Y"):?>
			<div style="margin-top: 10px; font-size: 13px;">График учитывает только средства, поступившие на счет Фонда, год рассчитывается с июня по май.</div>
		<?endif;?>
	  </div>
	</div>
	<? endif ?>
